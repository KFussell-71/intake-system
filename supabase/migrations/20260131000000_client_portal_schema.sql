...
-- ============================================================================
-- 0. PREREQUISITE: Ensure assigned_to column exists on clients
-- ============================================================================
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE clients ADD COLUMN assigned_to UUID REFERENCES profiles(id);
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Portal client can view own link" ON client_users;
...
DROP POLICY IF EXISTS "Staff can view assigned portal_activity" ON portal_activity;
DROP POLICY IF EXISTS "Portal clients can log own activity" ON portal_activity;
DROP POLICY IF EXISTS "System can log portal activity" ON portal_activity;
...
-- ============================================================================
-- 6. PORTAL-SPECIFIC DOCUMENT RLS POLICIES (if documents table exists)
-- ============================================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'documents') THEN
        ...
        EXECUTE 'CREATE POLICY "Portal clients can view own documents" ON documents
            FOR SELECT TO authenticated
            USING (
                EXISTS (
                    SELECT 1 FROM client_users
        ...
        EXECUTE 'CREATE POLICY "Portal clients can upload own documents" ON documents
            FOR INSERT TO authenticated
            WITH CHECK (
                EXISTS (
                    SELECT 1 FROM client_users
...
-- ============================================================================
-- 8. EXPLICIT DENY POLICIES FOR SENSITIVE TABLES (if tables exist)
-- Defense in depth: Even if application layer fails, DB blocks access
-- ============================================================================

DO $$
BEGIN
    -- Only create policies if report_versions table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'report_versions') THEN
        DROP POLICY IF EXISTS "Deny portal access to report_versions" ON report_versions;
        
        EXECUTE 'CREATE POLICY "Deny portal access to report_versions" ON report_versions
    ...
    -- Only create policies if report_reviews table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'report_reviews') THEN
        DROP POLICY IF EXISTS "Deny portal access to report_reviews" ON report_reviews;
        
        EXECUTE 'CREATE POLICY "Deny portal access to report_reviews" ON report_reviews
            FOR ALL TO authenticated
            USING (
                EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid())
            )';
...
-- ============================================================================
-- 10. STORAGE POLICIES FOR PORTAL DOCUMENT UPLOADS (if bucket exists)
-- ============================================================================

-- Note: Storage policies may need to be applied separately via Supabase dashboard

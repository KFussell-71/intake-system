...
export class DashboardRepository extends BaseRepository {
    ...
    async getMyWorkload(): Promise<MyWorkload | null> {
        const { data, error } = await this.db.rpc('analytics_get_my_workload');
    ...
    async getRecentActivity(limit: number = 20): Promise<ActivityFeedItem[]> {
        const { data, error } = await this.db.rpc('analytics_get_recent_activity', { limit_count: limit });
    ...
    async getIntakeTrends(days: number = 30): Promise<IntakeTrend[]> {
        const { data, error } = await this.db.rpc('analytics_get_intake_trends', { range_days: days });
        if (error) this.handleError(error, 'getIntakeTrends');
        return data || [];
    }

    async getStaffWorkload(): Promise<StaffWorkload[]> {
        const { data, error } = await this.db.rpc('analytics_get_staff_workload');
        if (error) this.handleError(error, 'getStaffWorkload');
        return data || [];
    }

    async getAnalyticsSummary(): Promise<any> {
        const { data, error } = await this.db.rpc('analytics_get_intake_analytics');

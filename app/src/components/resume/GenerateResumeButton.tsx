...
import { createResumeAction } from '@/app/(app)/actions/resumeActions';
...
export function GenerateResumeButton({ intakeId, clientId, clientName, onResumeGenerated }: Props) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const saveResult = await createResumeAction(clientId, {
                intakeId: intakeId,
                resumeId: resumeId,
                resumeUrl: resumeUrl,
                pdfUrl: pdfUrl,
            ...
            if (!saveResult.success) {
                throw new Error(saveResult.error || 'Failed to save resume record');
            }
    ...
    return (
        <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-2 ${success
                ...
                : 'bg-linear-to-r from-blue-600 to-indigo-600 hover:shadow-xl text-white disabled:opacity-50 disabled:cursor-not-allowed'
                }`}
        >
            {loading ? (
                <>

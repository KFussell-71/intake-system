...
import { getClientResumesAction, deleteResumeAction } from '@/app/(app)/actions/resumeActions';
import { ClientResume } from '@/repositories/ResumeRepository';
...
export function ResumeList({ clientId, refreshTrigger }: Props) {
    ...
    const [stats, setStats] = useState<{ total: number; active: number; lastGenerated?: Date } | null>(null);
    ...
    const loadResumes = async () => {
        try {
            const result = await getClientResumesAction(clientId);
            if (result.success && result.data) {
                setResumes(result.data as any);
                const active = result.data.filter((r: any) => r.isActive).length;
                const last = result.data.length > 0 ? (result.data[0] as any).createdAt : undefined;
                setStats({
    ...
    const handleDelete = async (resumeId: string) => {
        ...
        try {
            const result = await deleteResumeAction(resumeId, clientId);
            if (result.success) {
            ...
            {resumes.length === 0 ? (
                ...
                <div className="grid gap-3">
                    {resumes.map((resume) => (
                        <GlassCard
                            key={resume.id}
                            className={`p-4 transition-all hover:shadow-md ${resume.isActive ? 'ring-2 ring-primary/20' : 'opacity-75'
                                }`}
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {resume.metadata?.source === 'reactive-resume' && (
                                        <button
                                            onClick={() => window.open(reactiveResumeService.getEditorUrl(resume.resumeId), '_blank')}

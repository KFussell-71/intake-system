...
import { globalSearchAction } from '@/app/(app)/actions/searchActions';

/**
 * Client Search Dropdown (Refactored)
 * 
 * Migrated from direct Supabase browser client to globalSearchAction (Server Action).
 * This ensures all searches are audited and session-validated on the server.
 */
...
export function ClientSearchDropdown({ onClose }: Props) {
    ...
    useEffect(() => {
        ...
        const searchClients = async () => {
            ...
            setLoading(true);
            try {
                // Using the refactored Server Action instead of direct Supabase query
                const result = await globalSearchAction(searchTerm, 'clients', 10);
                
                if (isMounted && result.success && result.results) {
                    // The Server Action already maps the latest_intake into metadata
                    setClients(result.results.map(r => r.metadata));
                }
            } catch (err) {

...
export async function generateCaseNote(rawInput: string, type: string, clientName: string) {
    ...
    try {
        console.log(`[V3] Generating ${isClinicalNote ? 'Clinical Note' : 'Resource Plan'} via IntelligenceController`);

        const resultText = await IntelligenceController.execute({
            prompt: systemPrompt + "\n\n" + userPrompt,
            temperature: isClinicalNote ? 0.3 : 0.1,
            isPHISensitive: isClinicalNote, // PHI sensitive triggers local-only enforcement

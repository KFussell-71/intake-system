    ...
    const loadData = async () => {
        try {
            ...
            if (c) {
                ...
                // 3. Parallel Fetch: Notes, Clinical Notes & Outcomes
                const [notesRes, clinicalRes, measuresRes, historyRes] = await Promise.all([
                    supabase
                        .from('case_notes')
                        .select('*, author:profiles(username)')
                        .eq('client_id', c.client_id)
                        .order('created_at', { ascending: false }),
                    clinicalNoteService.getNotesForClient(c.client_id),

...
import { OutcomeMeasure, OutcomeRecord } from '@/services/OutcomeService';
...
import { ClinicalNoteForm } from '@/features/case-management/components/ClinicalNoteForm';
import { ClinicalNoteList } from '@/features/case-management/components/ClinicalNoteList';
...
import { ProviderDirectory } from '@/features/case-management/components/ProviderDirectory';
import { ReferralTracker } from '@/features/case-management/components/ReferralTracker';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { toast } from 'sonner';
import { FileText, ClipboardList, Activity, LayoutDashboard, Stethoscope, TrendingUp, MessageCircle, Users, Briefcase, History as HistoryIcon } from 'lucide-react';
...
import { getCaseDetailsAction } from '@/app/(app)/actions/caseDetailsActions';
    ...
    const loadData = async () => {
        try {
            ...
            if (result.success && result.data) {
                ...
                setCaseData(result.data.caseData as any);
                setNotes(result.data.notes as any);
                setClinicalNotes(result.data.clinicalNotes as any);
                setOutcomeMeasures(result.data.outcomeMeasures as any);
                setOutcomeHistory(result.data.outcomeHistory as any);
            } else {
                toast.error(result.error || 'Failed to loaded case details');
            }
        } catch (error) {
        ...
        <div className="container mx-auto px-6 py-8 pb-32">
            ...
            <Tabs defaultValue="overview" className="mt-8">
                <TabsList className="bg-slate-100 dark:bg-slate-800/50 p-1 mb-6 flex flex-wrap h-auto gap-2">
                    ...
                    <TabsTrigger value="clinical" className="gap-2 px-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <ClinicalNoteList
                                notes={clinicalNotes}
                                onSelect={(note) => {
                                    toast.info(`Note from ${new Date(note.created_at).toLocaleDateString()} selected`, {
                                        description: "View/Edit functionality is being prioritized."
                                    });
                                }}
                            />
                        </div>

...
import { scrubPII, scrubObject } from '@/lib/security/piiScrubber';
...
export interface IntakeBundle {
    ...
    employment?: any;
    observations?: any[];
    barriers?: { name: string; category: string }[];
    documents: {
        id: string;
        name: string;
        type: string;
...
`;
    ...
    - Primary Diagnosis: ${data.intake.primary_diagnosis_code || 'Not documented'}
    ...
    - ISP Action Plan: ${(data.employment?.isp_goals || []).map((g: any) => g.goal).join(', ')}
    - Observations: ${data.observations?.map((o: any) => `[${o.source}] ${o.observation_text}`).join('; ')}
    - Barriers: ${data.barriers?.map(b => b.name).join(', ') || 'None reported'}
    - Education: ${data.employment?.education_level || 'High School Diploma'}
    - Supportive Services: ${data.supportive_services.map(s => s.service_type).join(', ')}
    
    SIGNATURE STATUS:
    - Client Signature: ${data.intake.signature ? 'Captured (Base64)' : 'Pending'}
  ...
  `;
    ...
    try {
        ...
        // BLUE TEAM: Ensure fallback is also scrubbed of PII
        return generateMockReport(scrubObject(data) as IntakeBundle);
    }
}

function generateMockReport(data: IntakeBundle): string {
    // SECURITY: PII scrubbing is applied at the call site for this function in the catch block
    return `
# Participant Employment Services Intake Report

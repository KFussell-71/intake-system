...
const SEED_INTAKES = [
    {
        ...
        report_date: '2026-02-01', created_at: '2026-02-01T08:00:00Z',
        clients: { name: 'Kyla Stevenson' }, profiles: { username: 'James Jones' }
    }
];
...
const SEED_PORTAL_ACCESS = [
    {
        id: 'mock-user-id', client_id: 'mock-client-id',
        is_active: true, expires_at: '2027-01-01T00:00:00Z', created_at: '2026-01-01T00:00:00Z'
    }
];

const SEED_MILESTONES = [
    {
        id: 'm1', client_id: 'mock-client-id', milestone_name: 'Intake Completed',
        completion_date: '2026-02-01T08:00:00Z', step_order: 1, description: 'Initial diagnostic assessment'
    },
    {
        id: 'm2', client_id: 'mock-client-id', milestone_name: 'IPE Development',
        completion_date: '2026-02-15T14:30:00Z', step_order: 2, description: 'Individualized Plan for Employment'
    },
    {
        id: 'm3', client_id: 'mock-client-id', milestone_name: 'Job Search Training',
        completion_date: null, step_order: 3, description: 'Resume building and interview prep'
    }
];

const SEED_DOC_REQUESTS = [
    {
        id: 'dr1', client_id: 'mock-client-id', name: 'High School Diploma',
        description: 'Copy of your final diploma or GED certificate',
        status: 'pending', requested_at: '2026-02-10T10:00:00Z'
    }
];
...
class MockDataManager {
    ...
    private getInitialState(): MockState {
        return {
            ...
            client_users: [...SEED_PORTAL_ACCESS],
            tracking_milestones: [...SEED_MILESTONES],
            document_requests: [...SEED_DOC_REQUESTS]
        };
    }
    ...
    getTable(table: string): any[] {
        ...
        if (table === 'client_users') return this.state.client_users || [];
        if (table === 'tracking_milestones') return this.state.tracking_milestones || [];
        if (table === 'document_requests') return this.state.document_requests || [];
        return [];
    }
...
export const createMockSupabase = () => {
    return {
        ...
        rpc: async (fn: string, args: any) => {
            ...
            if (fn === 'analytics_get_my_workload') {
                const intakes = mockManager.getTable('intakes');
                return {
                    data: [{
                        active_clients: intakes.length,
                        intakes_in_progress: intakes.filter(i => i.status === 'draft' || i.status === 'In Progress').length,
            ...
            if (fn === 'analytics_get_recent_activity') {
                return {
                    data: [
                        { id: 1, type: 'intake', description: 'New intake started for Kyla Stevenson', created_at: new Date().toISOString() },
                        { id: 2, type: 'document', description: 'Referral Form uploaded', created_at: new Date(Date.now() - 3600000).toISOString() }
                    ],
                    error: null
                };
            }

            if (fn === 'analytics_get_intake_trends') {
                return {
                    data: [
            ...
            if (fn === 'analytics_get_staff_workload') {
                return {
                    data: [
                        { worker_name: 'James Jones', active_clients: 12, intakes_in_progress: 4 },
                        { worker_name: 'Sarah Smith', active_clients: 8, intakes_in_progress: 2 }
                    ],
                    error: null
                };
            }
        ...
        from: (table: string) => {
            const queryBuilder: any = {
                ...
                range: () => queryBuilder,
                single: async () => {
                    if (queryBuilder._lastResult) return queryBuilder._lastResult;
                ...
                insert: (data: any) => {
                    ...
                    queryBuilder._lastResult = { data: newData, error: null };
                    return queryBuilder;
                },
                update: (data: any) => {
                    console.log(`[MOCK] Updating ${table}`, data);
                    // Mock update logic: just return dummy success
                    queryBuilder._lastResult = { data: null, error: null };
                    return queryBuilder;
                },
                delete: () => {
            ...
            (queryBuilder as any).then = async (resolve: any, reject: any) => {
                if (queryBuilder._lastResult) {
                    resolve(queryBuilder._lastResult);
                    return;
                }
                let data = mockManager.getTable(table);
                if (queryBuilder._filters) {

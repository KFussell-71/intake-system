...
import { FileText, FolderOpen, FileUser } from 'lucide-react';
...
import { getClientReportDataAction } from '@/app/(app)/actions/getClientReportDataAction';
...
/**
 * ReportView Component (Refactored)
 * 
 * Migrated from direct Supabase browser client to secure Server Action data fetching.
 * Consolidates metadata fetching and ensures all operations are session-validated.
 */
export function ReportView({ clientId }: ReportViewProps) {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const result = await getClientReportDataAction(clientId);
            if (result.success && result.data) {
                setUserId(result.data.userId);
                setClientData(result.data.client);
                setPortalAccess(result.data.portalAccess);
                setLatestIntakeId(result.data.latestIntakeId);
            }
        } catch (err) {
            console.error('Failed to load report view metadata:', err);
        } finally {
            setLoading(false);
        }
    }, [clientId]);
    ...
    if (loading) {
        return <div className="p-12 text-center animate-pulse text-slate-500 font-bold">Loading Case File...</div>;
    }

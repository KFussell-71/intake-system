...
import { markNotificationReadAction } from '@/app/(app)/actions/notificationActions';
import { useSession } from 'next-auth/react';

/**
 * Notification Center Component (Refactored)
 * 
 * Migrated from direct Supabase browser client to useRealtimeNotifications hook
 * which utilizes Server Actions and session-aware polling.
 */
export const NotificationCenter = () => {
    const { data: session } = useSession();
    const { notifications, unreadCount, loading, refresh } = useRealtimeNotifications(session?.user?.id);

    const markAsRead = async (id: string) => {
        const result = await markNotificationReadAction(id);
        if (result.success) {
            refresh(); // Trigger a sync
        }
    };
                        ...
                        <AnimatePresence initial={false}>
                            {notifications.map((n) => (
                                <motion.div
                                    ...
                                    className={`p-4 border-b border-slate-50 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors relative group/item ${!n.isRead ? 'bg-primary/5' : ''}`}
                                >
                                    <div className="flex gap-3">
                                        <div className={`p-2 rounded-lg shrink-0 ${n.type === 'DOCUMENT_UPLOAD' ? 'bg-blue-500/10 text-blue-500' : 'bg-amber-500/10 text-amber-500'}`}>
                                            {n.type === 'DOCUMENT_UPLOAD' ? <FileText className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-xs leading-relaxed ${!n.isRead ? 'font-bold text-slate-900 dark:text-white' : 'text-slate-500'}`}>
                                                {n.message}
                                            </p>

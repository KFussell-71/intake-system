...
import { saveIdentityAction } from '@/app/(app)/actions/identityActions';
...
export function useIdentity(intakeId: string) {
    ...
    const fetchIdentity = useCallback(async () => {
        try {
            setLoading(true);
            // 1. Fetch Intake Data
            // We try to join intake_identity, but if the relationship is missing (PGRST200), we fallback.
            ...
            // Fallback for missing relationship or table
            if (intakeError && (intakeError as any).code === 'PGRST200') {
                console.warn('[useIdentity] Relational join failed (PGRST200), falling back to index-only fetch');
                const fallback = await supabase
                    ...
                    .eq('id', intakeId)
                    .single();
                intake = fallback.data ? { ...fallback.data, intake_identity: [] } : null;
    ...
    const saveIdentity = async (startData?: Partial<IdentityData>) => {
        ...
        try {
            ...
            const { saveIdentityAction } = await import('@/app/(app)/actions/identityActions');

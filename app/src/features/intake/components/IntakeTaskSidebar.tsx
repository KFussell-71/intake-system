...
import { IntakeTask, updateTaskStatusAction, createTaskAction, getIntakeTasksAction } from '@/app/(app)/actions/taskActions';
...
export const IntakeTaskSidebar: React.FC<Props> = ({ intakeId, isOpen, onToggle }) => {
    ...
    const fetchTasks = async (isInitial = false) => {
        ...
        try {
            const result = await getIntakeTasksAction(intakeId);
    ...
    useEffect(() => {
        ...
        // POLL FALLBACK: Every 30s for background updates

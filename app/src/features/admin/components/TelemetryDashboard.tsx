...
export const TelemetryDashboard: React.FC = () => {
    ...
    const latencyData = events
        .filter(e => e.eventName.startsWith('duration_'))
        .slice(0, 20)
        .reverse()
        .map(e => ({
            time: new Date(e.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            ms: e.value,
            name: e.eventName.replace('duration_', '')
        }));
    ...
    const errorDistribution = [
        { name: 'AI Proxy', value: events.filter(e => e.eventName.includes('AI') && e.eventType === 'error').length },
        { name: 'Sync', value: events.filter(e => e.eventName.includes('sync') && e.eventType === 'error').length },
        { name: 'Database', value: events.filter(e => e.eventName.includes('DB') && e.eventType === 'error').length },
        { name: 'Other', value: events.filter(e => e.eventType === 'error' && !['AI', 'sync', 'DB'].some(k => e.eventName.includes(k))).length },
    ].filter(d => d.value > 0);

    const riskDistribution = [
        { name: 'Routine', value: events.filter(e => e.eventName === 'clinical_urgency_score' && e.value <= 20).length },
        { name: 'Elevated', value: events.filter(e => e.eventName === 'clinical_urgency_score' && e.value > 20 && e.value <= 50).length },
        { name: 'High', value: events.filter(e => e.eventName === 'clinical_urgency_score' && e.value > 50 && e.value <= 80).length },
        { name: 'Critical', value: events.filter(e => e.eventName === 'clinical_urgency_score' && e.value > 80).length },
    ];
        ...
        <div className="space-y-8 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <MetricCard
                    ...
                    value={events.filter(e => e.eventName === 'sync_queue_size')[0]?.value?.toString() || '0'}
                        ...
                        <div className="w-[40%] flex flex-col justify-center space-y-4">
                            {errorDistribution.map((d, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full" style={{ background: COLORS[i % COLORS.length] } as React.CSSProperties} />

...
import { getCaseCommunicationsAction, sendCommunicationMessageAction } from '@/app/(app)/actions/communicationActions';
...
export function MessageCenter({ caseId, clientId, recipientContact }: Props) {
    ...
    const loadMessages = useCallback(async () => {
        try {
            const result = await getCaseCommunicationsAction(caseId);
    ...
    useEffect(() => {
        ...
        // POLL FALLBACK: Every 10 seconds for messaging sync
        // Replaces direct Supabase subscriptions
    ...
    const handleSend = async () => {
        ...
        setIsSending(true);
        try {
            const result = await sendCommunicationMessageAction({
                case_id: caseId,
                client_id: clientId,
                type: messageType,
                content: newMessage,
    ...
    const handleMagicPolish = async () => {
        if (!newMessage.trim()) return;
        setIsPolishing(true);
        // Simulate AI Polish (In production this would call an AI Server Action)
        setTimeout(() => {
        ...
        <GlassCard className="h-[600px] flex flex-col p-0 overflow-hidden">
            ...
            <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="text-xs text-slate-400 mt-2 px-1 flex justify-between">
                    <span>Sending via: <strong>Unified Communications</strong></span>

...
export class IntakeService {
    ...
    async saveIntakeProgress(intakeId: string, data: Partial<IntakeFormData>, editComment?: string, expectedVersion?: number) {
        ...
        try {
            ...
            const entity = new IntakeEntity(intakeId, (raw.data as any) || {}, raw.status as any, [], raw.version);
    ...
    async saveAssessment(assessment: Partial<IntakeAssessment>) {
        if (!assessment.intakeId) throw new Error("Intake ID required");
    ...
    async fetchServerData(intakeId: string) {
        try {
            ...
            return {
                ...
                updatedAt: assessment?.updatedAt || intake?.updatedAt
            };
        } catch (error) {

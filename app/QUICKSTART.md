...
1. **Choose Demo or Production Mode**
   - Choose **Demo Mode** if you're just testing (Local SQLite/Prisma)
   - Choose **Production Mode** if you want durable storage (NAS-hosted Postgres)

2. **Enter Credentials (Production Mode only)**
   - You'll need the **DATABASE_URL** for your NAS-hosted Postgres instance.
   - Example: `postgresql://user:pass@nas-ip:5432/intake_db`
...
## Step 5: Database Migrations (V3.2 Only)

If you are running in Production Mode, ensure the database schema is up to date:
...
- Use the credentials provided during the initial setup or ask your administrator.
...
Delete the `prisma/.env` file and run `npm run setup` again.

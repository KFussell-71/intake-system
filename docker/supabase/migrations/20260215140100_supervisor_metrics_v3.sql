-- Migration: 20260215_supervisor_metrics_v3
-- Description: Updates get_supervisor_metrics RPC to include Pipeline Velocity.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- 5. Pipeline Velocity (NEW)
    -- Average days spent in current stage for active cases
    'pipeline_velocity', (
        SELECT jsonb_agg(
            jsonb_build_object(
                'stage', stage,
                'avg_days', round(avg(EXTRACT(DAY FROM (NOW() - stage_changed_at)))::numeric, 1),
                'case_count', count(*)
            )
        )
        FROM cases

-- Migration: 20260224_upgrade_bundle_for_clinical_split
-- Purpose: Include client_statements and intake_assessments in the intake bundle.
  ...
  SELECT jsonb_build_object(
    ...
    'intake', (
      SELECT jsonb_build_object(
        ...
        'details', i.data, -- Legacy fallback
    ...
    'statements', (
      ...
      FROM client_statements cs
      JOIN intakes i ON i.id = cs.intake_id
      WHERE i.client_id = p_client_id
      ORDER BY i.created_at DESC
      LIMIT 1
    ),
    'assessment_record', (
      SELECT row_to_json(ia)
      FROM intake_assessments ia
      JOIN intakes i ON i.id = ia.intake_id
      WHERE i.client_id = p_client_id
      ORDER BY i.created_at DESC
      LIMIT 1
    ),
